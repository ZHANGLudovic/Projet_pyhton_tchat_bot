...
if __name__ == '__main__':
    ...
    for i in range(len(files_names)):
        with open("Cleaned/"+str(files_names[i]), 'w', encoding="utf-8") as f1, open("speeches/"+str(files_names[i]),'r', encoding="utf-8") as f2:
            lst = f2.read()
            for j in range(len(lst)):
                f1.write(str(cancel_char_and_majuscule(lst[j],lst[j-1])))
    #print(vecteur_tf_idf("messieurs messieurs le president messieurs ", "Cleaned"))

    tfidf_lst = tf_IDF("Cleaned")
    all_words = recupere_les_termes("Cleaned")
    print("                   Bienvenu dans le tchatbot")
    ...
    print("-Taper 7: Si vous voulez posez une question au robot")
    print("-Taper 8: Pour fermer la fenetre")


    booleen = True
    while booleen :

        valeurs_numerique = int(input("Saisissez des chiffre entre 1 à 8  pour accéder au diverse fonctionnalité du tchatbot: "))
        ...
        elif valeurs_numerique == 3:
            text = ""
            for i in range (0,2):
                with open ("Cleaned/"+str(files_names[i]), 'r', encoding="utf-8") as f1:
                    f_text = f1.readline()
                    text += f_text + ' '
            with open("chirac/"+"Nominated.chirac", 'w', encoding="utf-8") as f2:
                f2.write(f_text)
            with open("chirac/"+"Nominated.chirac", 'r', encoding="utf-8") as f3:
                dico = tf(f3.readline())
                comp = 0
                word = ""
    ...
    #Hormis les mots dits « non importants », quel(s) est(sont) le(s) mot(s) que tous les présidents ont évoqués.
        ...
        elif valeurs_numerique == 7:
            question = str(input("Avez-vous une question ? "))
            mise_en_forme = mise_en_page(question)
            print(str(mise_en_forme) + str(reponse("Cleaned", "speeches", question, tfidf_lst, files_names)))
        elif valeurs_numerique == 8:
            booleen= False
        else:
